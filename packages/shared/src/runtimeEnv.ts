...
export function shouldCaptureZCodeToolEnvPassthroughKey(key: string): boolean {
  const upperKey = key.toUpperCase();
  // 遥测类 env（OTEL_*/ZCODE_TELEMETRY_*）永远不进入 tool env；BYOK fork 已无遥测消费方，
  // 但保持该防泄漏边界，防止未来误把端点/凭据注入 Bash / MCP 子进程。
  if (upperKey.startsWith("OTEL_") || upperKey.startsWith("ZCODE_TELEMETRY_")) {
    return false;
  }
