...
// BYOK fork：Coding Plan 官网购买 webview（专用 preload + 导航守卫）已随厂商登录/订阅体系整体移除。
...
function attachEmbeddedBrowserWindowOpenHandler(options: {
  ...
  options.guestWebContents.setWindowOpenHandler((details): WindowOpenHandlerResponse => {
    ...
    // BYOK fork：Coding Plan 购买页 popup 路由分支已移除；统一走外部打开判定与内部 tab 路由。
    if (
...
export function createBrowserWindow(options: {
  ...
  win.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    ...
    if (!isAllowedEmbeddedBrowserUrl(params.src ?? "about:blank")) {
      options.logger.warn(`[browser-pane] blocked unsupported webview url: ${params.src}`);
      event.preventDefault();
      return;
    }
  });
