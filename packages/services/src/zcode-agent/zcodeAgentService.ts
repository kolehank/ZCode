...
import type { UsageDeltaFactEvent } from "../usage-ledger/usageLedgerService.js";
...
> {
  ...
  /**
   * 本地用量账本（usageLedger）的旁路 sink：只收已校验的 `usage.delta` 事实
   * （CLI 侧 `model_request_completed` 的协议投影；`model_request_failed` 不产此事实）。
   * 与已删除的 ModelApiTelemetryStatusSink 同型的本地观测口——实现方必须内部消化异常，
   * 不能让账本失败阻断会话事件管道。缺省（纯 CLI / 测试装配）不接账本。
   */
  onConversationUsageDeltaFact?: (event: UsageDeltaFactEvent) => void;
}
      ...
      client.onNotification((message) => {
        ...
        if (message.method === V4_NOTIFICATIONS.conversationTelemetryFact) {
          ...
          if (parsed.success) {
            ...
            // 本地用量账本的旁路 sink：与 renderer 遥测订阅无关，host 装配了账本才接线；
            // sink 自吞异常（见 CreateZCodeAgentServiceOptions 注释），这里不 try/catch 兜底。
            if (parsed.data.kind === "usage.delta" && options?.onConversationUsageDeltaFact) {
              options.onConversationUsageDeltaFact({
                workspaceKey: resolveWorkspaceKey(workspace),
                fact: parsed.data,
              });
            }
