...
import { createUsageLedgerService, IUsageLedgerService } from "./usage-ledger/usageLedgerService.js";
...
export function createLocalServices(options: {
  ...
  // OAuth 会话凭据不再存在；凭据变化只可能来自 Account Provider api-key 缓存。
  ...
  // 厂商 OAuth/ZCode JWT 登录已下线（BYOK）：apiClient 不再监听产品 JWT 失效。
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    // BYOK：Built-in Provider 配置只读本地文件；远端 CDN 刷新通道
    // 已随厂商云功能下线，不再注入 zcodeBuiltinEnvironment。
    zcodeBuiltinFilePath: options.zcodeBuiltinProviderConfigFilePath,
    zcodeBuiltinActiveFilePath: options.zcodeBuiltinProviderConfigFilePath,
    onZCodeBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "ZCode Built-in Config 刷新失败", { error });
    },
  ...
  // 厂商账号接入源（OAuth/coding plan）已下线：账号 Overlay 固定 fail-closed，
  // Built-in Config 也不再声明 account: Provider。空源无可释放资源，无需 dispose。
  ...
  // 本地 token 用量账本（BYOK P5）：旁路 sink 接在 agent service 的 usage.delta 事实入口上，
  // 纯本地落盘（usage-ledger.sqlite），不参与任何上报；close 进 sqliteReposToClose dispose 链。
  const usageLedgerService = createUsageLedgerService();
  const zcodeAgentService = createZCodeAgentService({
    // 本地用量账本（usageLedger）：只消费已校验的 usage.delta 事实；sink 内部自吞异常。
    onConversationUsageDeltaFact: (event) => usageLedgerService.recordUsageDelta(event),
    accountProviderConfigSource,
    ...(modelSelectionReadinessSource ? { modelSelectionReadinessSource } : {}),
    authorizeLocalMediaPreviewPath: options?.authorizeLocalMediaPreviewPath,
    // 动态工作流灰度：远端配置通道已随厂商云功能下线，
    // 这里保留最小占位——只按本地覆盖环境变量解析（dev/preview 可用，生产缺省 disabled）。
  ...
  const services = new ServiceCollection()
    ...
    .register(IUsageLedgerService, usageLedgerService)
  ...
  // usage-ledger.sqlite 是独立连接句柄，同链收口（Windows 上悬着句柄会撞 EBUSY）
  sqliteReposToClose.push(usageLedgerService);
